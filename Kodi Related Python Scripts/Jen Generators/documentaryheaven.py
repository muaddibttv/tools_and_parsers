...
class Generator:
    def __init__( self ):
        ...
        self.output = 'main.xml'

        # CHANGEME if url changes for the website.
        self.url = 'http://documentaryheaven.com/watch-online/'
    ...
    def _generate_templates ( self ):
        ...
        try:
            ...
            lists = parseDOM(html, 'ul', attrs={'class':'cat-list'})

            count = 0
            for list in lists:
                items = parseDOM(list, 'li')
                for item in items:
                    ...
                    output_string = output_string + '    <docuh>dhcategory/' + str(url) + '</docuh>\n'
                    if not self.fanart == None:
                        output_string = output_string + '    <fanart>' + str(self.fanart) + '</fanart>\n'
                    output_string = output_string + '</dir>\n\n'
                    count += 1

            print('Completed Documentary Category entries for ' + str(count) + ' categories')
        except:
            failure = traceback.format_exc()
            print('Documentary Heaven - Exception: \n' + str(failure))
